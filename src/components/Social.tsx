    ...
    <section ref={containerRef} className="relative py-32 overflow-hidden" id="contact">
      ...
      <div className="container mx-auto px-6 relative z-10">
              ...
              >
                <div className="w-full h-full img-placeholder">
                  <span className="text-xs text-muted text-center">
                    ...
                    <br />
                    (400 x {index === 0 || index === 3 ? "533" : "400"}px)
                  </span>
                </div>
                {/* Hover overlay */}
        ...
        <ScrollReveal delay={0.3}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 p-8 bg-card rounded-2xl border border-border">
            <p className="text-lg text-muted">Follow me on social media</p>
            ...
            <div className="flex items-center gap-6">
              {socialLinks.map((social) => (
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="group relative p-3 rounded-xl bg-background border border-border hover:border-accent/50 transition-colors"
                >
                  <span className="text-muted group-hover:text-accent transition-colors">
                    {social.icon}
                  </span>
                  {/* Tooltip */}
                  <span className="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-foreground text-background text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    {social.name}
                  </span>
                </motion.a>
              ))}
